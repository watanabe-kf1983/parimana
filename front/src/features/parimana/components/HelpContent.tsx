...
## あらまし
parimanaは、公営競技で、**各出走者の能力**が投票者からどう見積もられているか、公表されているオッズから算出します。  
...
二種類の算出方法をクリックで選択できます。  
どちらの方法も厳密な計算ではなく、ゆるふわです。   

 * **さっくりモデル**  
さっくりと計算しています。  
具体的には、各出走者どうしの成績の相関関係を無視しているのがふんわりモデルとの違いです。

 * **ふんわりモデル**  
ふんわりと計算しています。  
具体的には、各出走者どうしの成績の相関関係をオッズから仮定しているのがさっくりモデルとの違いです。
...
オッズからゆるふわ計算で推定された競走能力として、出走者ごとの走破時計の確率分布を示しています。  
...
### 出走者間の相関（ふんわりモデルのみ）
オッズから推定された各出走者の成績の相関関係を図で示したものです。
位置が近い番号どうしは正相関、離れた番号どうしは負相関があると推定されます。
...
当サイトは著作権を主張しません。これらは当サイトにより算出されたものですが、著作権の対象とすべき創作性はないためです。ちかぢか算出に用いているプログラムソースが公開されれば、数値やグラフは当サイトでなくても、ほんのちょっとの計算機コストで算出可能になるものです。
