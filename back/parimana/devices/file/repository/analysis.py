...
class FileAnalysisRepository(FileRepositoryBase, AnalysisRepository):
    ...
    def save_charts(self, race: Race, timestamp: OddsTimeStamp, charts: AnalysisCharts):
        ...
        utils.write_as_pickle(dir_ / "charts.pickle", charts)
        utils.write_bytes(dir_ / "result.xlsx", charts.excel)
        utils.write_html_chart(dir_ / "oc.html", charts.odds_chance)
        utils.write_html_chart(dir_ / "box-plot.html", charts.model_box)
    ...
    def load_charts(self, race: Race, ts: OddsTimeStamp, model: str) -> AnalysisCharts:
        ...
        return utils.read_pickle(dir_ / "charts.pickle")
    ...
    def save_latest_odds_time(self, race: Race, ts: OddsTimeStamp) -> None:
        utils.write_as_pickle(self._race_dir(race) / "odds_ts.pickle", ts)

    def load_latest_odds_time(self, race: Race) -> Optional[OddsTimeStamp]:
        return utils.read_pickle(self._race_dir(race) / "odds_ts.pickle")

    def save_latest_charts_time(self, race: Race, ts: OddsTimeStamp) -> None:
        utils.write_as_pickle(self._race_dir(race) / "charts_ts.pickle", ts)

    def load_latest_charts_time(self, race: Race) -> Optional[OddsTimeStamp]:
        return utils.read_pickle(self._race_dir(race) / "charts_ts.pickle")
